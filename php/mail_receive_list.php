...
<table class="table table-bordered table-striped">
    ...
    <tr>
        ...
        <th style="width: 60px;"><img src='../images/mail0.svg' alt='' style='width: 30px;height: 25px;'></th>
    ...
    <tbody>
      ...
      $user_id=$_SESSION["userid"];
   		 $sql="select email_id,email_state,email_sender,email_name,email_date from email where email_addressee='".$user_id."' and email_type=1 and and is_use_ad=1 order by email_date desc";
        ...
        $sql1 = "select * from email where email_addressee='".$user_id."' and email_type=1 and is_use_ad=1"; 
        ...
        // echo $count;
       $sql="select email_id,email_state,email_sender,email_name,email_date from email where email_addressee='".$user_id."' and is_use_ad=1 and email_type=1 order by email_date desc limit " . (($pageNum - 1) * $pageSize) . "," . $pageSize; //
  		...
  		while($row=mysqli_fetch_array($rs)){
  			...
  			echo "<td><a href='mail_view.php?email_id=$row[0]' title=''><img src='../images/mail$row[1].svg' alt='' style='width: 30px;height: 25px;'></a></td>";
			...
			echo "<td>$row1[0]</td>";
  			...
  			for($i=3;$i<5;$i++){
  				...
  				echo "</td>";
  			   	...
  			   	echo "<td><a href='mail_view.php?email_id=$row[0]' title=''>$row[$i]</a></td>";
  			   }
  			}
  			echo "</tr>";
  		}
...
<input type="hidden" name='act'>
    ...
    <input type="submit" name="return" value="标为已读" onclick="document.forms['view_form'].act.value='checked';"/>
    <input type="submit" name="return" value="全部标为已读" onclick="document.forms['view_form'].act.value='allchecked';"/>
</form>
