...
<?php 
  ...
  // 	}
  	...
  	$sql2="select job_number,is_use from user";
 	$rs2=mysqli_query($conn,$sql2);
 	while($row2=mysqli_fetch_array($rs2)){
  		if($row2[0] == $userId && $row[1] == 1)
  		{
  			$flag2 = 0;
  			break;
  		}
  	...
  	if($flag2 == 0){
  		showAlert("该工号已注册");
		pageLocator("staff_managemen.php");
  	}
	...
	$sql1="insert into enterprise_staff(job_number,name) values(";
	$sql1.="'".$userId."','".$userName."')";
	$rs1=mysqli_query($conn,$sql1);
	//echo $sql1;

	$sql="insert into user(job_number,user_name,user_sex,user_phone,user_address,user_workdate,user_emailbox,user_image,is_use) values(";
	$sql.="'".$userId."','".$userName."',";
